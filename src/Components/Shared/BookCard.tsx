...
const BookCard = ({book} :IBookProps) => {
    ...
    <div className="relative flex h-72 items-center justify-center overflow-hidden bg-gray-100 p-6">
        ...
        {/* Category */}
        <span className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-xs font-bold text-green-700 shadow-sm backdrop-blur">
            {book.category}
        </span>

        {/* Rating */}
        <span className="absolute right-4 top-4 rounded-full bg-white/90 px-3 py-1 text-sm font-semibold text-gray-800 shadow-sm backdrop-blur">
            ⭐ {book.rating}
        </span>
    </div>
    ...
    <div className="p-5">
        ...
        {/* Title */}
        <h2 className="line-clamp-1 text-xl font-bold text-gray-900 transition-colors group-hover:text-green-700">
            {book.bookName}
        </h2>
        ...
        {/* Button */}
        <button className="mt-5 w-full rounded-xl bg-green-600 py-3 text-sm font-semibold text-white transition hover:bg-green-700">
            View Details
        </button>
